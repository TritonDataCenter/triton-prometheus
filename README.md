...
and [Grafana](https://github.com/TritonDataCenter/triton-grafana) to track
their own metrics and monitor themselves. All metrics are gathered via
...
document](https://github.com/TritonDataCetner/manta/blob/master/docs/mantav2.md)
...
Triton has suspended development on Prometheus and Grafana services for use in
Triton and Manta.
...
First ensure that [CMON](https://github.com/TritonDataCenter/triton-cmon) and
[CNS](https://github.com/TritonDataCenter/triton-cns) are set up in your
TritonDC, typically via:
...
[Manta Operator's Guide](https://github.com/TritonDataCenter/manta/blob/master/docs/operator-guide/maintenance.md#upgrading-manta-components)
...
[CNS](https://github.com/TritonDataCenter/triton-cns). To allow queries from an
arbitrary number of CNS servers and offload some traffic from CNS, the
Prometheus zone runs a BIND server that resolves the CNS names using zone
transfer. The BIND server forwards all requests outside of the CNS zone to
external and binder resolvers. BIND runs on localhost, and localhost is the
only entry in /etc/resolv.conf.

## Security

Prometheus listens on the admin and external networks. The firewall with the
[standard Triton rules](https://github.com/TritonDataCenter/sdc-headnode/blob/34dbd8acd65523c844385a81239ea0a872750326/scripts/headnode.sh#L188-L228)
