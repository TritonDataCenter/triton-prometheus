...
curl -sS -k --max-time 120 \
    ...
    | json cns \
    | json -e "this.targets=[this.server_uuid+'.${CMON_HOST}:9163']; this.server_uuid=undefined; this.labels={'job': 'global_zones_${DATACENTER_NAME}', 'hostname': this.server_hostname}; this.server_hostname=undefined;" \
    > ${OUTPUT_FILE_TMP}
